...
class Tank
    ...
    attr_accessor :id, :x_pos, :y_pos, :angle, :turret_rotation, :speed, :health, :fire

    # An id value of -1 indicated invalid tank.
    def initialize(id = -1, x = 0, y = 0, angle = 0, turret_rotation = 0, speed = 0, health = 3, fire = false)
        @id = id
        @x_pos = x
        @y_pos = y
        @angle = angle
    ...
    def jsonify(action)
        ...
        tank = {:action => action, :id => @id,:x_pos => @x_pos,
                ...
                :speed => @speed, :health => @health, :fire => @fire
        }
